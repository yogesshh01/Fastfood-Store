...
function Profile({ cart }) {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#FAF8F5] dark:bg-gray-950 flex flex-col items-center justify-center px-4 pt-28 pb-16 font-sans">
        <div className="text-center">
          <div className="w-24 h-24 bg-red-50 dark:bg-red-950/30 rounded-full flex items-center justify-center mx-auto mb-6 border dark:border-red-900/30">
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] dark:bg-gray-950 pt-28 pb-16 font-sans">
      <div className="max-w-5xl mx-auto px-4 md:px-8">
        ...
        {/* Profile Header Card */}
        <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm p-8 mb-6 flex flex-col sm:flex-row items-center sm:items-start gap-6">
          {/* Avatar */}
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-red-400 to-red-600 flex items-center justify-center text-white font-black text-4xl shadow-lg shadow-red-200 dark:shadow-red-950/30 shrink-0 animate-fade-in">
            {initial}
          </div>
          ...
          <div className="flex-1 text-center sm:text-left">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              <h1 className="text-2xl font-black text-gray-900 dark:text-white capitalize">{user.name}</h1>
              <span className="inline-block bg-green-50 dark:bg-green-950/30 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-900/30 text-xs font-bold px-3 py-1 rounded-full">
                ✓ Verified Member
              </span>
            </div>
            <p className="text-gray-400 dark:text-gray-400 text-sm mt-1 flex items-center justify-center sm:justify-start gap-1.5">
              <FaEnvelope className="text-xs" /> {user.email}
            </p>
            <p className="text-gray-400 dark:text-gray-400 text-xs mt-0.5 flex items-center justify-center sm:justify-start gap-1.5">
          ...
          <div className="flex gap-3 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-red-50 dark:bg-red-950/30 hover:bg-red-100 dark:hover:bg-red-900/20 text-red-500 dark:text-red-400 font-bold text-sm px-4 py-2.5 rounded-xl transition-all cursor-pointer active:scale-95"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          {/* JWT Token Display */}
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-emerald-100 dark:border-emerald-950/30 shadow-sm p-5 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-7 h-7 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                  <FaShieldAlt className="text-xs" />
                </div>
                <h3 className="font-bold text-gray-800 dark:text-white text-sm">Active JWT Session</h3>
                <span className="ml-auto bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900/30 text-xs font-bold px-2.5 py-0.5 rounded-full">● Active</span>
              </div>
              <div className="bg-gray-50 dark:bg-gray-950 rounded-xl p-3 border border-gray-100 dark:border-gray-800">
                <p className="text-[11px] font-mono text-gray-500 dark:text-gray-450 break-all leading-relaxed">{tokenPreview}</p>
              </div>
            </div>
            <p className="text-xs text-gray-400 dark:text-gray-400 mt-2">Token expires in 7 days. Stored securely in localStorage.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Recent Orders */}
          <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
            ...
            <div className="flex flex-col gap-3">
              {recentOrders.map((order, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-950 rounded-2xl border border-gray-100 dark:border-gray-800">
                  <div>
                    <p className="font-bold text-gray-800 dark:text-white text-sm">{order.name}</p>
                    <p className="text-gray-400 dark:text-gray-400 text-xs mt-0.5">{order.date}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-gray-800 dark:text-white text-sm">{order.price}</p>
                    <span className="text-xs text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-950/30 border dark:border-green-900/30 px-2 py-0.5 rounded-full font-semibold">{order.status}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
              ...
              {cartCount === 0 ? (
                ...
                <>
                  <p className="text-gray-500 dark:text-gray-400 text-xs mb-1">{cartCount} item{cartCount > 1 ? "s" : ""} in cart</p>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
              <h3 className="font-bold text-gray-800 dark:text-white text-sm mb-4">Quick Links</h3>
